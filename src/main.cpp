...
uint256 hashGenesisBlock("0x932b5f81f991266201cc5b9405c3f8fff6ffecb5d37ccbcd6876dcfb51a92e15");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Quickcoin: starting difficulty is 1 / 2^12
...
{
    // Quickcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Quickcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Quickcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Quickcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Quickcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Quickcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xea515f42369cb5fb19f9409c6d1f2dd9a040af1046c3ac7cb443b161f60959b4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 08/14/2020 If You Recover From Virus, You’re Protected for 3 Months, C.D.C. Says";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f9d86a71d9a695490a4c5b5ba359c5f1ddd794028e50675b132c9ee354e10ab9f4971b12faeaa06a8ba9818ef88b822bc944accb12363216c076256cf9d973b5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1597422424;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085359074;

        if (fTestNet)
        {
            block.nTime    = 1597422410;
            block.nNonce   = 385449273;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcb78a58552c531fd74c4a0c45fbc8cb5d886e39563bdab7d001717a9119573be"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xda }; // Quickcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QuickcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static QuickcoinMiner(CWallet *pwallet)
{
    printf("QuickcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running QuickcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("QuickcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&QuickcoinMiner, pwallet));
}
